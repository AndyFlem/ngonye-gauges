...
const COLUMNS = ['datetime', 'level_zra', 'level_wpc', 'count', 'level_average']
...
// FTP mount vs. the data disk), so fall back to copy+delete in that case. copyFileSync
// is avoided here because it uses copy_file_range under the hood, which this setup's
// NFS mount rejects with EACCES instead of the EXDEV/EOPNOTSUPP libuv expects to retry
// on - sometimes after leaving a 0-byte stub at dest, owned by whoever ran it. Unlinking
// any stale dest first sidesteps that stub (directory write access is enough to remove
// a file we don't own) and a plain read+write avoids copy_file_range entirely.
...
function buildMonthGrid(monthKey) {
  ...
  while (cursor <= monthEnd) {
    ...
    rows.set(key, { datetime: key, level_zra: '', level_wpc: '', count: '', level_average: '' })
...
function recomputeRow(row) {
  const values = []
  if (isValidValue(String(row.level_zra))) values.push(parseFloat(row.level_zra))
  if (isValidValue(String(row.level_wpc))) values.push(parseFloat(row.level_wpc))
  row.count = values.length
  row.level_average = values.length ? fmt(d3.mean(values)) : ''
}
...
function main() {
  ...
  // readings: monthKey -> (datetimeStr -> { level_zra?, level_wpc? })
  ...
  for (const [monthKey, monthReadings] of readings) {
    ...
    if (fs.existsSync(file)) {
      ...
      for (const r of existing) {
        ...
        if (r.level_zra !== undefined && r.level_zra !== '') row.level_zra = r.level_zra
        if (r.level_wpc !== undefined && r.level_wpc !== '') row.level_wpc = r.level_wpc
      }
    }
    ...
    for (const [dtStr, cells] of monthReadings) {
      ...
      if (cells.level_zra !== undefined) row.level_zra = fmt(cells.level_zra)
      if (cells.level_wpc !== undefined) row.level_wpc = fmt(cells.level_wpc)
      rowsTouched++
    }
